...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    if (len > MAX_TRANSACTION_LENGTH + 2)
...
u32 gtp_read_ID_version(struct i2c_client *client)
{
    ...
    {
	GTP_DEBUG("[TSP-xw]the [%d] is 0x%08x ...\n", i, (int)buf[i]);
    }
    return ret;
}
...
{	
    unsigned int tp_fw_version = tpd_info.vid;

    GTP_DEBUG("[TSP-xw]:tp_fw_version is 0x%08x\n",tp_fw_version);
    return(tp_fw_version);
}
unsigned int tpd_read_id_version(void)
{
    unsigned int tp_id_version = tpd_info.mid;

    GTP_DEBUG("[TSP-xw]:tp_id_version is 0x%08x\n",tp_id_version);	
...
#if GTP_COMPATIBLE_MODE
    u8  rqst_data[4] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
#endif

#if GTP_SLIDE_WAKEUP
    u8 doze_buf[4] = {0x81, 0x4B};
    u8 doze_buf_double[4] = {0x81,0x4D};//lenovo-sw add for double clic
#endif
#if 0 //G_DEBUG
...
#ifdef CONFIG_LENOVO_POWEROFF_CHARGING_UI
		if(((input_x>LENOVO_CHARGING_DRAW_LEFT)&&(input_x<LENOVO_CHARGING_DRAW_RIGHT))&&
		   ((input_y>LENOVO_CHARGING_DRAW_TOP)&&(input_y<LENOVO_CHARGING_DRAW_BOTTOM))&&
		   (ipo_flag ==0x1) &&(g_tp_poweron !=0x1))
		{
		    g_tp_poweron = 0x1;
		    tp_button_flag = 0x1;   
...
#if GTP_SLIDE_WAKEUP
    GTP_DEBUG("[TSP-xw]enter in slide mode\n");
    tpd_suspend_status = get_tpd_suspend_status();
...
static void tpd_resume(struct early_suspend *h)
{
    ...
    doze_status = DOZE_DISABLED;
    tpd_suspend_status = get_tpd_suspend_status();
    if (tpd_suspend_status && (lpwg_flag == 1))
    {
	lpwg_flag = 0;
	lpwg_int_flag = 0;
    }
